...
class VGG(nn.Module):
    ...
    def forward(self, x,mode='output', target_class = [None]):
        ...
        # at this point, we are at between the feature extractor and the classifier
        
        # backpropagate until the desired layer is reached
        for i in range(len(self.features)-1, int(mode), -1):
            R = self.features[i].relprop(R)

        # average the featuremap 
        r_weight = torch.mean(R, dim=(2, 3), keepdim=True)
        # weighted feature maps
        r_cam = target_layer * r_weight

        #weighted average is the resulting relevance cam
