...
def skresnext50_32x4d(pretrain=True):
    return timm.create_model('skresnext50_32x4d', pretrained=pretrain)
...
model_dict = {
    ...
    'skresnext50_32x4d': [skresnext50_32x4d, 2048]
}
...
class SimClrSkResneXt(nn.Module):
    ...
    def forward(self, x):
        feat = self.encoder(x)
        feat = F.normalize(self.head(feat), dim=1) # NOTE BENEFIT MORE FOR THE Xent CL loss
        return feat
